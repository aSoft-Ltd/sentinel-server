...
class EmailRegistrationScopedServiceFlix(private val options: EmailRegistrationScopedServiceFlixOptions) : EmailRegistrationService {
    ...
    override fun sendVerificationLink(params: SendVerificationLinkParams): Later<String> = options.scope.later {
        ...
        sender.send(options.verification.factory(fp, null, options.brand)).await()
        tracer.passed()
        params.email
    }
    ...
    override fun createUserAccount(params: UserAccountParams): Later<UserAccountParams> = options.scope.later {
        ...
        val event = EmailRegistrationCompletedEvent(
            name = candidate.name,
            email = params.loginId,
            scope = options.parent,
            password = params.password
        )
        options.bus.dispatch(options.topic.completed(pbr.person.toHexString()), event)
        tracer.passed()
        params
    }
